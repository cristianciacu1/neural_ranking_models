...
class MsmarcoIterator(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        line = linecache.getline(self.data_dir, index + 1).strip()
        
        # Assuming TSV format: _id, title, text
        parts = line.split('\t')
        
        example = {
            'id': parts[0],
            'text': self.sep.join([parts[1]])
        }
        return example
...
def msmarco(data_dir: str = None):
    assert data_dir is not None, "Please add a data_dir"
    return MsmarcoIterator(os.path.join(data_dir, 'collection.tsv'))
